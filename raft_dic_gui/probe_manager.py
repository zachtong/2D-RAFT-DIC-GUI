...
class Probe:
    ...
    def to_dict(self):
        """Serialize probe to a JSON-compatible dict."""
        coords = self.coords
        ...
        if isinstance(coords, tuple):
            ...
            coords = [
                [float(c) for c in pt] if isinstance(pt, (list, tuple)) else pt
                for pt in coords
            ]
        elif isinstance(coords, dict):
            # Area probe: {'shape': str, 'data': list}
            data = coords.get("data", [])
            if isinstance(data, list) and data and isinstance(data[0], (list, tuple)):
                data = [[float(c) for c in pt] for pt in data]
            elif isinstance(data, list):
                data = [float(c) if isinstance(c, (int, float, np.integer, np.floating)) else c for c in data]
            coords = {"shape": coords["shape"], "data": data}
        return {
            "id": self.id,
            "type": self.type,
            "coords": coords,
    ...
    @classmethod
    def from_dict(cls, d):
        """Deserialize probe from a dict."""
        coords = d["coords"]
        probe_type = d["type"]
        # Restore coords to expected format
        if probe_type == "point":
            coords = tuple(coords)  # list -> tuple
        elif probe_type == "line":
            coords = [tuple(pt) for pt in coords]
        # area coords stay as dict {'shape': ..., 'data': ...}
        return cls(d["id"], probe_type, coords, d["color"], d.get("label"))
...
class ProbeManager:
    ...
    def to_list(self):
        """Serialize all probes to a list of dicts."""
        return [p.to_dict() for p in self.probes]

    def load_from_list(self, data_list):
        """Restore probes from a list of dicts."""

...
\title{\textbf{RAFTcorr User Manual v1.0}}
...
\begin{itemize}
    \item \textbf{Mode}: Currently supports \textbf{Accumulative} mode, where every frame is correlated against the first (reference) frame. Incremental mode is currently disabled.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Background Image Mode}: Toggle between underlaying the "Reference Image" (static) or the "Deformed Image" (dynamic). (Currently, only supports the static mode)
    \end{itemize}
